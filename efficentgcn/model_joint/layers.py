...
class Basic_Tem_Layer(nn.Layer):
    def __init__(self, in_channel, out_channel, residual, bias, act, **kwargs):
        ...
        self.conv = nn.Conv2D(in_channel+16, out_channel, 1)
        self.bn = nn.BatchNorm2D(out_channel)
        self.tem_embed = embed(500, 16, norm=False, bias=bias)
        group = np.gcd(in_channel + 1, in_channel)
        self.residual = nn.Sequential(
            nn.Conv2D(in_channel + 16, in_channel, 1, (1, 1), groups=group),
            nn.BatchNorm2D(in_channel),
        )
        # self.residual = Identity() if residual else Zero_Layer()
        self.act = act
...
class Temporal_Tem_Basic_Layer(Basic_Tem_Layer):
    def __init__(self, channel, temporal_window_size, bias, stride=1, residual=True, **kwargs):
        ...
        padding = (temporal_window_size - 1) // 2
        self.conv = nn.Conv2D(16+channel, channel, (temporal_window_size, 1), (stride, 1), (padding, 0))
        if residual and stride != 1:
            self.residual = nn.Sequential(
                nn.Conv2D(channel+16, channel, 1, (stride, 1)),
                nn.BatchNorm2D(channel),
            )
...
class cnn1x1(nn.Layer):
    def __init__(self, dim1=3, dim2=3, bias=True):
        ...
        self.cnn = nn.Conv2D(dim1, dim2, kernel_size=1, bias_attr=bias)
