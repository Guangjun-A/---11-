...
class Classifer(nn.Layer):
    ...
    def forward(self, x):
        return self.drop_out(self.fc(x))
...
def stack_boosting(args):
    model_args = args.model_args
    K_fold = args.kfold
    assert args.eval_model_path != None, 'MISS eval model path !'
    assert args.config != None, 'Miss config file'

    checkpoint_path = f'./{args.eval_model_path}'
    print("checkpoint_path: ", checkpoint_path)
    model_name = args.model  # f'net.{args.model}.Model'
    ...
    if not os.path.exists(boost_data_npy_path):
        ...
        for k in range(K_fold):
            ...
            model = load_model(model, root_path=checkpoint_path, k=k, load_best=True)
    ...
    if not args.test:  # 训练分类器
        ...
        #
        # optimizer = Momentum(parameters=FC_model.parameters(),
        #                            learning_rate=scheduler,
        ...
        for epoch in range(fc_train_epoch):
            ...
            for i, (boost_x, label) in enumerate(tqdm(fc_train_dataloader)):
                ...
                # label = paddle.nn.functional.label_smooth(label)  # .cuda()
                y = FC_model(boost_x)
        ...
        print('-' * 50 + 'testing...' + '-' * 50)
        #     if acces_f1_score.avg * 100 > best_f1_acc:
        #         best_acc = acces.accumulate() * 100
        #         best_f1_acc = acces_f1_score.avg * 100
        #         best_epoch = epoch
        #     paddle.save(FC_model.state_dict(), os.path.join(linear_checkpoint_path, f"checkpoint_best.pdparams"))
        #     np.save(os.path.join(checkpoint_path, 'y_val_score.npy'), y_score)
        #     log_info('epoch: {:<3d} lr:{:.10f} loss: {:.5f} acc: {:.2f}, f1 acc: {:.2f} '
        #              ' acc {:.2f} best f1 acc {:.2f} best epoch {:<3d}'.format(epoch, lr,
        #                                                                        losses.avg,
        #                                                                        acces.accumulate() * 100,
        #                                                                        acces_f1_score.avg * 100,
        #                                                                        best_acc,
        #                                                                        best_f1_acc,
        #                                                                        best_epoch), log_txt_file)
        #
        # print('-' * 50 + 'testing...' + '-' * 50)
