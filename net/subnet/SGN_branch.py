...
class SGN(nn.Layer):
    def __init__(self, seg, channel=4, out_channel=32, dim_embed=32, **kwargs):
        ...
        self.gcn2 = gcn_spa(self.dim_embed * 2, self.dim_embed * 4, bias=bias)
        self.compute_g2 = compute_g_spa(self.dim_embed * 2, self.dim_embed * 4, bias=bias)
        self.ms_tcn2 = MultiScale_TemporalConv(self.dim_embed * 4, out_channel)
        self.tem_embed2 = embed(self.seg, self.dim_embed * 4, norm=False, bias=bias)  # 64 * 4 # 和out_channel一致

        self.gcn3 = gcn_spa(self.dim_embed * 4, self.dim_embed * 8, bias=bias)
        self.compute_g3 = compute_g_spa(self.dim_embed * 2, self.dim_embed * 4, bias=bias)
        # self.ms_tcn3 = MultiScale_TemporalConv(self.dim_embed * 8, self.dim_embed * 8)
        #
        self.gcn4 = gcn_spa(self.dim_embed * 8, self.dim_embed * 4, bias=bias)
        self.compute_g4 = compute_g_spa(self.dim_embed * 2, self.dim_embed * 8, bias=bias)
...
class gcn_spa(nn.Layer):
    ...
    def forward(self, x1, g):  # X: N, C, V ,T
        ...
        x = self.w(x) + self.w1(x1) + res # + x_physical_A + res  # + res  # gcn1 (N, 128, V, T), gcn2 (N, 256, V, T)
...
if __name__ == '__main__':
    ...
    # args = True
    # print(args.train)
    model = SGN(500, channel=4, out_channel=48)
    # print(model)
    bs, step, dim, V = 2, 500, 4, 25  # （num_skes, max_num_frames, 75)
    # [10, 144, 20, 25]
