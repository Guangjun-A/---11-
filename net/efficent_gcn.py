...
from efficentgcn.model_joint.layers import SpatialGraphConvJoint, Temporal_Basic_Layer, Spatial_Graph_Layer, Spatial_Graph_Joint_Layer
import paddle
...
class EfficientGCN(nn.Layer):
    def __init__(self, data_shape, block_args, fusion_stage, stem_channel, input_type='JVB', **kwargs):
        super(EfficientGCN, self).__init__()

        num_input, num_channel, _, _, _ = data_shape
        num_input = len(input_type)
        ...
        if 'J' in input_type:
            ...
            self.input_branches = self.input_branches.extend(self.joint_input_branches)
        # input branches
        if 'V' in input_type:
            self.vec_input_branches = nn.LayerList([EfficientGCN_Blocks_Joint(
                ...
                input_channel=num_channel,
                **kwargs)])
            self.input_branches = self.input_branches.extend(self.vec_input_branches)

        if 'B' in input_type:
            self.bone_input_branches = nn.LayerList([EfficientGCN_Blocks(
                init_channel=stem_channel,
                block_args=block_args[:fusion_stage],
                input_channel=num_channel,
                **kwargs)])
            self.input_branches = self.input_branches.extend(self.bone_input_branches)  #.extend(self.vec_input_branches).extend(self.bone_input_branches)

        if 'A' in input_type:
            self.angle_input_branches = nn.LayerList([EfficientGCN_Blocks(
                init_channel=stem_channel,
                block_args=block_args[:fusion_stage],
                input_channel=num_channel,
                **kwargs)])
            self.input_branches = self.input_branches.extend(self.angle_input_branches)  # .extend(self.vec_input_branches).extend(self.bone_input_branches)
...
class EfficientGCN_Blocks_Joint(nn.Sequential):
    def __init__(self, init_channel, block_args, layer_type, kernel_size, input_channel=0, **kwargs):
        ...
        if input_channel > 0:  # if the blocks in the input branches
            self.add_sublayer('init_bn', BN_Layer(input_channel))
            self.add_sublayer('stem_scn',
                              Spatial_Graph_Joint_Layer(input_channel, init_channel, max_graph_distance, **kwargs))
            self.add_sublayer('stem_tcn', Temporal_Basic_Layer(init_channel, temporal_window_size, **kwargs))
        ...
        for i, [channel, stride, depth] in enumerate(block_args):
            self.add_sublayer(f'block-{i}_scn',
                              Spatial_Graph_Joint_Layer(last_channel, channel, max_graph_distance, **kwargs))
            for j in range(depth):
                s = stride if j == 0 else 1
                self.add_sublayer(f'block-{i}_tcn-{j}',
                                  temporal_layer(channel, temporal_window_size, stride=s, **kwargs))
...
class Model(nn.Layer):
    def __init__(self, config, input_mode):
        ...
        self.model = self.create(input_mode, args['model_type'], **args['model_args'], **kwargs)
    ...
    def create(self, input_mode, model_type, act_type, block_args, scale_args,  **kwargs):
        kwargs.update({
...
if __name__ == "__main__":
    ...
    # }
    #
    model = Model('../configs/4_branch_eff_v2.yaml', 'JVB')
    y = model(data)
