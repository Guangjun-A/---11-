...
from efficentgcn.model_joint_tem.layers import Spatial_Tem_Graph_Layer, Temporal_Tem_Basic_Layer,SpatialGraphConvJoint, Spatial_Graph_Joint_Tem_Layer, Temporal_Basic_Layer, Spatial_Graph_Layer, \
...
class EfficientGCN(nn.Layer):
    def __init__(self, data_shape, block_args, fusion_stage, stem_channel, input_type, **kwargs):
        ...
        if 'B' in input_type:
            self.bone_input_branches = nn.LayerList([EfficientGCN_Blocks_Joint(
                ...
                input_channel=num_channel,
                **kwargs)])
            self.input_branches = self.input_branches.extend(self.bone_input_branches)

        if 'A' in input_type:
            self.angle_input_branches = nn.LayerList([EfficientGCN_Blocks_Joint(
                init_channel=stem_channel,
                block_args=block_args[:fusion_stage],
                input_channel=num_channel,
                **kwargs)])
            self.input_branches = self.input_branches.extend(self.angle_input_branches)
...
class EfficientGCN_Main_Blocks(nn.Sequential):
    def __init__(self, init_channel, block_args, layer_type, kernel_size, frame_seg, input_channel=0, **kwargs):
...
class Model(nn.Layer):
    def __init__(self, config, input_mode, seg=500):
...
if __name__ == "__main__":
    ...
    # }
    #
    model = Model('../configs/4_branch_eff_with_tem_angle.yaml', 'JVB')
    y = model(data)
