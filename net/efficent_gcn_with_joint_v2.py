...
class EfficientGCN(nn.Layer):
    def __init__(self, data_shape, block_args, fusion_stage, stem_channel, **kwargs):
        ...
        self.bone_input_branches = nn.LayerList([EfficientGCN_Blocks(
            ...
            input_channel=num_channel,
            **kwargs)])
        self.input_branches = self.joint_input_branches.extend(self.vec_input_branches).extend(self.bone_input_branches)
...
if __name__ == "__main__":
    ...
    # }
    #
    model = Model('../configs/efficentgcn_joint_v2.yaml', 'JVB')
    y = model(data)
