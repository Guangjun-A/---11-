...
class FsdDataset(Dataset):
    def __init__(self, data_path, label_path, angle_path, window_size=350, inputs='JVB', center_point=1, PCA=False,
        ...
        # self.angle = self.sample_data(self.angle, window_size=window_size)
...
def GetKDataLoader(args, file=None, window_size=350, K=10, debug=False, local_debug2=False, parallel=True):
    ...
    test_data_path = os.path.join(path, 'remove_zeros_test_B_data.npy')
    angle_train_path = os.path.join(path, 'remove_zeros_shutil_train_angle3.npy')
    angle_test_path = os.path.join(path, 'remove_zeros_test_B_angle3.npy')
...
def GetKDataLoader_test(args, file=None, window_size=350, K=10, debug=False, local_debug2=False, parallel=True):
    path = 'data/new_data'
    data_path = os.path.join(path, 'remove_zeros_shutil_train_data.npy')
    label_path = os.path.join(path, 'shutil_train_label.npy')
    test_data_path = os.path.join(path, 'remove_zeros_test_B_data.npy')
